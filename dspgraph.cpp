/**************************************************************************
File Name		: DSPGRAPH.CPP			| Author		: JOHN EDWARDS
...
---------------------------------------------------------------------------
Description : Functions to display a graphical buffer.
...
//	wxMessageBox (_T("Got here"), _T("LineGraph::LineGraph"), wxOK | wxICON_EXCLAMATION, NULL);
	...
	VSizer0->Add (TopLabel = new wxStaticText  (this, -1, "Max"), 0, wxALIGN_TOP | wxALIGN_LEFT | wxALL, 5);
	VSizer0->Add (0, 0, 1);									// This line pads out the vertical space
	VSizer0->Add (MiddleLabel = new wxStaticText  (this, -1, "Middle"), 0, wxALIGN_CENTER | wxALIGN_LEFT | wxALL, 5);
	VSizer0->Add (0, 0, 1);									// This line pads out the vertical space
	VSizer0->Add (BottomLabel = new wxStaticText  (this, -1, "-Max"), 0, wxALIGN_BOTTOM | wxALIGN_LEFT | wxALL, 5);

	HSizer1->SetMinSize (graphSize.GetWidth()+80, -1);		// Set min width for horizontal text

						// Horizontal axis labels
	HSizer1->Add (new wxStaticText  (this, -1, "             "), 0, wxALIGN_LEFT | wxALL, 5);
	HSizer1->Add (LeftLabel = new wxStaticText  (this, -1, "0"), 0, wxALIGN_LEFT | wxALL, 0);
	...
	HSizer1->Add (0, 0, 1);									// This line pads out the horizontal space
	HSizer1->Add (RightLabel = new wxStaticText  (this, -1, "N                 "), 0, wxALIGN_RIGHT | wxALL, 0);
...
#define MINOR_MARK_SIZE		4
									...
									// Horizontal dividers
#define VERT_NUMBER_OF_DIVISIONS	8L		// Split top and bottom into quarters
#define VERT_MARKS_PER_DIVISION		4L
#define VERT_TOTAL_NUMBER_OF_MARKS	(VERT_NUMBER_OF_DIVISIONS * VERT_MARKS_PER_DIVISION)

	for (i = 1L; i < VERT_TOTAL_NUMBER_OF_MARKS; i++)
	{
...
//	wxMessageBox (_T("Got here"), _T("LineGraph::Draw"), wxOK | wxICON_EXCLAMATION, NULL);
	...
	for (i = 1L; i < 40L; i++)
	{
		...
		if ((i & 3L) == 0L)
		{
			GraphPaintDC.DrawLine ((int)((i * Width) / 40L), 0, (int)((i * Width) / 40L), Height);
		}
		...
		{
									// Top
			GraphPaintDC.DrawLine ((int)((i * Width) / 40L), 0, (int)((i * Width) / 40L), MAJOR_MARK_SIZE);
									// Middle
			GraphPaintDC.DrawLine ((int)((i * Width) / 40L), (Height >> 1) - (MAJOR_MARK_SIZE - 1),
										(int)((i * Width) / 40L), (Height >> 1) + MAJOR_MARK_SIZE);
									// Bottom
			GraphPaintDC.DrawLine ((int)((i * Width) / 40L), Height - MAJOR_MARK_SIZE, (int)((i * Width) / 40L), Height);
		}
									// Minor mark
		else
		{
									// Top
			GraphPaintDC.DrawLine ((int)((i * Width) / 40L), 0, (int)((i * Width) / 40L), MINOR_MARK_SIZE);
									// Middle
			GraphPaintDC.DrawLine ((int)((i * Width) / 40L), (Height >> 1) - (MINOR_MARK_SIZE - 1),
										(int)((i * Width) / 40L), (Height >> 1) + MINOR_MARK_SIZE);
									// Bottom
			GraphPaintDC.DrawLine ((int)((i * Width) / 40L), Height - MINOR_MARK_SIZE, (int)((i * Width) / 40L), Height);
		}
	}
//	wxMessageBox (_T("Got here"), _T("LineGraph::Draw"), wxOK | wxICON_EXCLAMATION, NULL);
...
*	void
*
* Description : Display a buffer of double fmt data
...
//	wxMessageBox (_T("Got here"), _T("LineGraph::AddDataSet"), wxOK | wxICON_EXCLAMATION, NULL);
	...
	if (scaleFlag == GRAPH_SCALE)				// Auto scale graph
	{
		...
		if (Max == 0.0L)						// Do not divide by 0
		{
			Max = 1.0L;
		}
	}
...
//	wxMessageBox (_T("Start"), _T("PoleZeroPlot::PoleZeroPlot"), wxOK | wxICON_EXCLAMATION, NULL);
	...
	VSizer->Add (PlotPanel, 0, wxALIGN_CENTER | wxALIGN_RIGHT | wxALL, 5);
